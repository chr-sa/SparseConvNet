...
if testReps > 1:
    test_data = [
        test_data[x]
        for i in range(0, 654, testBatchSize // testReps)
        for _ in range(testReps)
        for x in range(i, min(i + testBatchSize // testReps, 654))
    ]
...
def train_merge(tbl):
    ...
    return (
        scn.batch_location_tensors(locations_),
        torch.cat(features_, 0),
        torch.cat(targets_, 0),
    )
...
trainIterator = torch.utils.data.DataLoader(
    train_data,
...
def ShrinkScatterC22l(dimension, nPlanes, nClasses, reps, depth=4):
    ...
    def bar(nPlanes, bias):
        ...
        m.add(scn.BatchNormReLU(nPlanes))
        m.add(
            scn.NetworkInNetwork(nPlanes, nClasses, bias)
    ...
    def baz(depth, nPlanes):
        if depth == 1:
            ...
            return (
                ...
                .add(
                    scn.ConcatTable()
                    .add(bar(nPlanes, False))
                    .add(
                        scn.Sequential()
                        .add(scn.BatchNormReLU(nPlanes))
                        .add(
                            scn.Convolution(dimension, nPlanes, l(nPlanes), 2, 2, False)
                        )
...
if p["check_point"] and os.path.isfile("epoch.pth"):
    ...
    print("Restarting at epoch " + str(p["epoch"]))
...
for epoch in range(p["epoch"], p["n_epochs"] + 1):
    ...
    print(
        ...
        stats["c"] / stats["n"],
        "MegaMulAdd=",
        scn.forward_pass_multiplyAdd_count / 795 / 1e6,
        "MegaHidden",
        scn.forward_pass_hidden_states / 795 / 1e6,
        "time=",
        time.time() - start,
        "s",
    )
