...
class Model(nn.Module):
    def __init__(self):
        ...
        self.sparseModel = (
            scn.Sequential()
            .add(scn.InputLayer(data.dimension, data.full_scale, mode=4))
            .add(scn.SubmanifoldConvolution(data.dimension, 3, m, 3, False))
            .add(
                scn.UNet(
                    data.dimension,
                    block_reps,
            ...
            .add(scn.OutputLayer(data.dimension))
        )
...
for epoch in range(training_epoch, training_epochs + 1):
    ...
    print(
        ...
        "MegaHidden",
        scn.forward_pass_hidden_states / len(data.train) / 1e6,
        "time=",
        time.time() - start,
        "s",
    )
