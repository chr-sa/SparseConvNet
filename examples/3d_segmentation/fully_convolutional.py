...
class Model(nn.Module):
    def __init__(self):
        ...
        self.sparseModel = (
            scn.Sequential()
            .add(scn.InputLayer(dimension, data.spatialSize, mode=3))
            .add(scn.SubmanifoldConvolution(dimension, 1, m, 3, False))
            .add(
                scn.FullyConvolutionalNet(
                    dimension, reps, nPlanes, residual_blocks=False, downsample=[3, 2]
                )
            )
            .add(scn.BatchNormReLU(sum(nPlanes)))
...
if p["check_point"] and os.path.isfile("epoch.pth"):
    ...
    print("Restarting at epoch " + str(p["epoch"]) + " from model.pth ..")
...
def store(stats, batch, predictions, loss):
    ctr = 0
    for nP, f, classOffset, nClasses in zip(
        batch["nPoints"], batch["xf"], batch["classOffset"], batch["nClasses"]
    ):
        categ, f = f.split("/")[-2:]
        ...
        stats[categ][f]["p"] += (
            predictions.detach()[ctr : ctr + nP, classOffset : classOffset + nClasses]
            .cpu()
            .numpy()
        )
        stats[categ][f]["y"] = (
            batch["y"].detach()[ctr : ctr + nP].cpu().numpy() - classOffset
        )
        ctr += nP
...
def inter(pred, gt, label):
    ...
    return np.sum(
        np.logical_and(pred.astype("int") == label, gt.astype("int") == label)
    )
    ...
    for i, categ in enumerate(categories):
        ...
        for j in range(npart):
            ...
            for k in range(len(pred)):
                p = pred[k]
                iou_per_part[k, j] = (inter(p, gt[k], j) + eps) / (
                    union(p, gt[k], j) + eps
                )
...
for epoch in range(p["epoch"], p["n_epochs"] + 1):
    ...
    print(
        ...
        r["iou"],
        "MegaMulAdd=",
        scn.forward_pass_multiplyAdd_count / r["nmodels_sum"] / 1e6,
        "MegaHidden",
        scn.forward_pass_hidden_states / r["nmodels_sum"] / 1e6,
        "time=",
        time.time() - start,
        "s",
    )
