...
def train():
    ...
    def merge(tbl):
        ...
        xl_ = [
            np.hstack([x, idx * np.ones((x.shape[0], 1), dtype="int64")])
            for idx, x in enumerate(xl_)
        ]
        return {
            "x": [torch.from_numpy(np.vstack(xl_)), torch.from_numpy(np.vstack(xf_))],
    ...
    return torch.utils.data.DataLoader(
        d, batch_size=batchSize, collate_fn=merge, num_workers=10, shuffle=True
    )
