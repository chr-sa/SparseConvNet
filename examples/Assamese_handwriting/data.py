...
def interp(sample, x, y):
    return torch.from_numpy(
        np.hstack(
            [
                np.interp(sample.numpy(), x.numpy(), y[:, i].numpy())[:, None]
...
class Data(torch.utils.data.Dataset):
    def __init__(self, file, scale=63, repeats=1):
        ...
        if repeats > 1:
            print(
                "Replicating dataset: 1 epoch = %d iterations of the dataset; %d x %d = %d training samples"
                % (repeats, repeats, len(self.data), repeats * len(self.data))
            )
        for j in range(len(self.data)):
...
def TrainMergeFn(spatial_size=95, jitter=8):
    ...
    def merge(tbl):
        ...
        for idx, char in enumerate(tbl):
            ...
            coords = char["coords"]
            coords = torch.mm(coords, m) + torch.FloatTensor(1, 2).uniform_(
                center - jitter, center + jitter
            )
            coords = torch.cat(
                [coords.long(), torch.LongTensor([idx]).expand([coords.size(0), 1])], 1
            )
            ...
            features.append(f)
        return {
            "input": scn.InputLayerInput(
                torch.cat(locations, 0), torch.cat(features, 0)
            ),
...
def get_iterators(*args):
    return {
        "train": torch.utils.data.DataLoader(
            Data("pickle/train.pickle", repeats=10),
            collate_fn=TrainMergeFn(),
            batch_size=108,
            shuffle=True,
            num_workers=10,
        ),
        "val": torch.utils.data.DataLoader(
            Data("pickle/test.pickle", repeats=1),
            collate_fn=TestMergeFn(),
            batch_size=183,
            shuffle=True,
            num_workers=10,
        ),
