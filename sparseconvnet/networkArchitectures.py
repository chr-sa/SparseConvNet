...
def SparseVggNet(dimension, nInputPlanes, layers):
    ...
    for x in layers:
        if x == "MP":
            ...
            m.add(
                scn.ConcatTable()
                .add(scn.SubmanifoldConvolution(dimension, nPlanes, x[1], 3, False))
                .add(
                    scn.Sequential()
                        ...
                        .add(
                            ...
                            .add(
                                ...
                                if stride == 1
                                    ...
                                    dimension, nPlanes, n, 3, stride, False
                                )
                            )
    ...
    def U(nPlanes):  # Recursive function
        ...
        if len(nPlanes) == 1:
            ...
            m.add(
                ...
                .add(
                    scn.Sequential()
                    .add(scn.BatchNormReLU(nPlanes[0]))
                    .add(
                        scn.Convolution(
                            dimension,
                            nPlanes[0],
                    ...
                    .add(U(nPlanes[1:]))
                    .add(scn.UnPooling(dimension, downsample[0], downsample[1]))
                )
            )
            m.add(scn.JoinTable())
        return m
    ...
    def foo(m, np):
        for _ in range(reps):
            if residual:  # ResNet style blocks
                ...
                m.add(scn.BatchNormLeakyReLU(np, leakiness=leakiness)).add(
                    scn.SubmanifoldConvolution(dimension, np, np, 3, False)
                )
