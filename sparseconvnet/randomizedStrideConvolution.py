...
class RandomizedStrideConvolution(Module):
    ...
    def __init__(
        self, dimension, nIn, nOut, filter_size, filter_stride, bias, groups=1
    ):
    ...
    def forward(self, input):
        ...
        output.spatial_size = (input.spatial_size - self.filter_size).div(
            self.filter_stride, rounding_mode="trunc"
        ) + 1
        assert (
            (output.spatial_size - 1) * self.filter_stride + self.filter_size
            == input.spatial_size
        ).all()
        output.features = (
            RandomizedStrideConvolutionFunction
            if self.training
            else ConvolutionFunction
        ).apply(
    ...
    def __repr__(self):
        ...
        if (
            self.filter_size.max().item() == self.filter_size.min().item()
            and self.filter_stride.max().item() == self.filter_stride.min().item()
        ):
            s = (
                s
                + str(self.filter_size[0].item())
                + "/"
