...
class DenseToSparse(Module):
    ...
    def forward(self, input):
        ...
        output.features = DenseToSparseFunction.apply(
            input, output.metadata, output.spatial_size, self.dimension
        )
        return output
...
class DenseToSparseFunction(Function):
    @staticmethod
    def forward(ctx, input, output_metadata, output_spatial_size, dimension):
        ...
        output_metadata.createMetadataForDenseToSparse(
            output_spatial_size, nz.cpu(), input.size(0)
        )
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = (
            grad_output.new().resize_(ctx.aas2).zero_().index_copy_(0, r, grad_output)
        )
