...
class UnPoolingFunction(Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(input_spatial_size, output_spatial_size)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros(
            ctx.input_features_shape, device=grad_output.device, dtype=grad_output.dtype
        )
...
class UnPooling(Module):
    ...
    def __repr__(self):
        s = "UnPooling"
        if (
            self.pool_size.max() == self.pool_size.min()
            and self.pool_stride.max() == self.pool_stride.min()
        ):
            s = (
                s
